...
class Pizarra extends Service
{
    ...
    public function _main (Request $request)
    {
        if ($request->query == "reemplace este texto por su nota") {
            ...
            $responseContent = array(
                    "message" => 'Para que podamos escribir su nota, &iexcl;Usted primero debe escribirla!</p><p>Por favor presione el bot&oacute;n m&oacute;s abajo y reemplace en el asunto del email donde dice <b>"reemplace este texto por su nota"</b> con el texto a escribir e intente nuevamente.'
            );
        ...
        if (! empty($request->query)) {
            ...
            foreach ($mentions as $mention) {
                ...
                $responseContent = array(
                        "message" => "El usuario <b>@$user</b> le ha mencionado en una nota escrita en la pizarra. La nota se lee a continuaci&oacute;n:<br/><br/><br/>{$request->query}"
                );
                $response = new Response();
                $response->setResponseEmail($mention[1]); // email the user
                                                          // mentioned
            ...
            // post in tweeter
            $text = trim(str_replace(" @", " ", $text), "@"); // remove
                                                              // @usernames for
                                                              // twitter
            try {
            ...
            $responseContent = array(
                    "message" => "Hemos escrito su nota en la pizarra. Usted es ahora parte de la conversaci&oacute;n. $textMentioned"
            );
        ...
        $listOfNotes = $connection->deepQuery(
    			...
    			LEFT JOIN person B
    			ON A.email = B.email
    		    WHERE NOT EXISTS (SELECT * FROM _pizarra_locks WHERE _pizarra_locks.email = '{$request->email}' AND _pizarra_locks.user_locked = B.email)
    		    AND EXISTS (SELECT * FROM _pizarra_follow WHERE _pizarra_follow.email = '{$request->email}' AND _pizarra_follow.user_followed = B.email)
    			LIMIT 30
    			...
    			ON A.email = B.email
    		    WHERE NOT EXISTS (SELECT * FROM _pizarra_locks WHERE _pizarra_locks.email = '{$request->email}' AND _pizarra_locks.user_locked = B.email)
    		    AND NOT EXISTS (SELECT * FROM _pizarra_follow WHERE _pizarra_follow.email = '{$request->email}' AND _pizarra_follow.user_followed = B.email)
        ...
        foreach ($listOfNotes as $note) {
            ...
            $notes[] = array(
                    ...
                    "text" => $note->text,
                    // mysql server timezone must be in America/New_York
        ...
        for ($i = 0; $i < count($pizarra); $i ++) {
            $pizarra[$i]['text'] = ucfirst(strtolower($pizarra[$i]['text'])); // fix
                                                                              // case
    ...
    public function _buscar (Request $request)
    {
        ...
        foreach ($listOfNotes as $note) {
            ...
            $notes[] = array(
                    ...
                    "picture" => $note->picture,
                    "text" => $note->text,
                    "inserted" => date("Y-m-d H:i:s", strtotime($note->inserted)), // mysql
                                                                                   // server
                                                                                   // timezone
                                                                                   // must
                                                                                   // be
        ...
        for ($i = 0; $i < count($notes); $i ++) {
            $notes[$i]['text'] = ucfirst(strtolower($notes[$i]['text'])); // fix
                                                                          // case
    ...
    public function _reportar (Request $request)
    {
        ...
        // add one to the reports counter, or set up a penalization for 3 days
        // if the counter gets to 5
     ...
     * Subservice BLOQUEAR
     *
     * Function executed when a user want to lock another user's activity
     *
    ...
    public function _bloquear (Request $request)
    {
        ...
        if (empty($email . '')) {
            $response = new Response();
            $responseContent = array(
                    "message" => "El usuario $username no existe. Por favor verifica que hayas escrito bien el nombre de usuario que quieres bloquear. Si persiste el error consulta al soporte t&eacute;nico."
            );
            $response->setResponseSubject("El susuario $username no existe");
            $response->createFromTemplate("message.tpl", $responseContent);
            return $response;
        }
        
        $sql = "START TRANSACTION;
		DELETE FROM _pizarra_locks WHERE email = '{$request->email}' AND user_locked='{$email}';
		INSERT INTO _pizarra_locks (email, user_locked) VALUES ('{$request->email}','$email');
		COMMIT;";
        ...
        $response = new Response();
        $responseContent = array(
                "message" => "A partir de ahora el contenido del usuario $username no te ser&aacute; mostrado cuando uses el servicio PIZARRA."
        );
        $response->setResponseSubject("Te hemos bloqueado satisfactoriamente a $username");
        $response->createFromTemplate("message.tpl", $responseContent);
    ...
    public function _like (Request $request)
    {
        ...
        $sql = "INSERT INTO _pizarra_follow (email, user_followed) VALUES ('{$request->email}',(SELECT email FROM _pizarra_notes WHERE id='$id'));";
        $connection->deepQuery($sql);
        
        // create the response
        $response = new Response();
        $responseContent = array(
                "message" => 'Hemos agregado un coraz&oacute;n al post, lo cual adem&aacute;s se reflejar&aacute; en la reputaci&oacute;n de quien lo escribi&oacute;. &iexcl;Gracias por compartir su opini&oacute;n!'
        );
