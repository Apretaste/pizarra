...
class Service
{
	...
	private function getNotesByTopic($profile, $topic): array
	{
		...
		usort($listOfNotes, function ($a, $b) {
			$a->score = ($a->staff ? 0 : pow($a->hours, 1.5) * -1) + max($a->commentsUnique, 20) * 0.2 + (($a->likes - $a->unlikes * 2) * 0.4) + $a->ad * 1000 + ($a->staff ?? 0) * 99999;
			$b->score = ($b->staff ? 0 : pow($b->hours, 1.5) * -1) + max($b->commentsUnique, 20) * 0.2 + (($b->likes - $b->unlikes * 2) * 0.4) + $b->ad * 1000 + ($b->staff ?? 0) * 99999;
