...
class Service
{
    ...
    public function _nota(Request $request, Response $response)
    {
        ...
        Connection::query("UPDATE _pizarra_notes SET views=views+1 WHERE id={$note['id']}");
    ...
    public function _escribir(Request $request, Response &$response)
    {
        ...
        $topics = empty($topics[0]) ? [Connection::query("SELECT default_topic FROM _pizarra_users WHERE id_person='{$request->person->id}'")[0]->default_topic] : $topics[0];
    ...
    public function _comentar(Request $request, Response $response)
    {
        ...
        $note = Connection::query("SELECT `text`,id_person FROM _pizarra_notes WHERE id='$noteId' AND active=1");
        if ($note) {
            $note = $note[0];
        } else {
            return;
    ...
    public function _populares(Request $request, Response $response)
    {
        ...
        $populars = Connection::query('SELECT A.id_person, A.avatar, A.avatarColor, B.username, B.first_name, B.country, 
    ...
    {
        if ($toId != $fromId) {
            Connection::query("INSERT INTO _pizarra_reputation(id_person, id_from, id_note, amount) VALUES ($toId, $fromId, $noteId, $amount)");
        }
    }
    ...
    {
        $myUser = Connection::query("SELECT (SELECT SUM(amount) AS reputation 
        ...
        if (empty($myUser)) {
            ...
            Connection::query("INSERT IGNORE INTO _pizarra_users (id_person) VALUES ('{$profile->id}')");
            $myUser = Connection::query("SELECT reputation, avatar, avatarColor FROM _pizarra_users WHERE id_person='{$profile->id}'")[0];
        } else {
            $myUser = $myUser[0];
        }
    ...
    private function profileCompletion($profile)
    {
        ...
        if (!property_exists($profile, 'highest_school_level')) {
